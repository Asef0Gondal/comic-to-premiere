...
def validate_file_size(file, max_size_mb: int, file_type: str) -> Tuple[bool, Optional[str]]:
    \"\"\"Validate file size.\"\"\"
    if file is None:
...
def safe_api_call(func, *args, **kwargs):
    ...
    try:
        ...
        if \"quota\" in error_msg.lower():
            return None, \"API quota exceeded. Please check your Gemini API quota or try again later.\"
...
with st.sidebar:
    ...
    with st.expander(\"📊 System Info\", expanded=False):
        ...
        **Supported Formats:**
        - Images: {\", \".join(SUPPORTED_IMAGE_FORMATS)}
...
with col1:
    st.markdown('<h3 class=\"step-header\">🖼️ Panel Setup</h3>', unsafe_allow_html=True)
    
    subcol1, subcol2, subcol3 = st.columns([1, 1, 2])
    with subcol1:
...
with col2:
    st.markdown('<h3 class=\"step-header\">🎙️ Audio & Script</h3>', unsafe_allow_html=True)
...
if not st.session_state.processing_complete:
    ...
    if panels_uploaded == 0:
        ...
        missing.append(f\"✅ {panels_uploaded} panels uploaded\")
    ...
    if panel_errors:
        missing.append(f\"❌ {len(panel_errors)} file errors\")
    
    if audio_error:
        missing.append(\"❌ Audio error\")
    ...
    if st.button(\"Process & Generate XML\", type=\"primary\", disabled=not ready_to_process, use_container_width=True):
        ...
        with st.spinner(\"Processing...\"):
            ...
            try:
                ...
                processed_images = [] # (filename, bytes)
                image_filenames = []
                    ...
                    if panel is not None:
                        try:
                            ...
                            if cache_key in st.session_state.processed_images_cache:
                                ...
                                if error:
                                    st.error(f\"Panel {i+1} error: {error}\")
                                    processed = img_data # Use original image as fallback
...
- {len(processed_images)} panel images (panel_001.jpg, panel_002.jpg, etc.)
...
if st.session_state.processing_complete:
    ...
    with col1:
        ...
        with st.expander(\"📊 View Generated Timings\", expanded=False):
            st.json(st.session_state.timings)
            
        if st.session_state.processed_images_preview:
            with st.expander(\"🖼️ Preview Processed Images\", expanded=False):
    ...
    with col2:
        ...
        cols = st.columns(3)
        cols[0].metric(\"Duration\", f\"{total_duration:.1f}s\")
        cols[1].metric(\"Panels\", len(st.session_state.timings))
        cols[2].metric(\"Avg/Panel\", f\"{avg_duration:.1f}s\")
    ...
    *Tip: Keep all files in the same directory for proper linking.*
    \"\"\")
